...
class Pengaturan extends CI_Controller {
	...
	public function menu($filter1='', $filter2='', $filter3='')
	{
		if ($this->session->userdata('logged_in') == TRUE){
			...
			if ($data['action'] == 'view'){
				...
				$edit['menu_status']	= 'H';
				$this->ADM->update_menu($where_edit, $edit);
				$this->session->set_flashdata('warning','Data menu telah berhasil dihapus!,');
				redirect("pengaturan/menu");
			}
			$this->load->vars($data);
			$this->load->view('admin/home');
	...
	public function kelompok_pengguna($filter1='', $filter2='', $filter3='')
	{
		if ($this->session->userdata('logged_in') == TRUE){
			...
			@date_default_timezone_set('Asia/Jakarta');
			$data['dashboard_info']		= FALSE;
            $data['breadcrumb']         = 'Kelompok Pengguna';
			...
			if ($data['action'] == 'view'){
				...
				$where['admin_level_kode']	= $filter2; 
				...
				$this->ADM->update_admin_level($where_edit, $edit);
				$this->session->set_flashdata('warning','Kelompok pengguna telah berhasil dihapus!,');
				redirect("pengaturan/kelompok_pengguna");
			}
			$this->load->vars($data);
			$this->load->view('admin/home');
