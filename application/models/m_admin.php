...
class M_admin extends CI_Model  {
	...
	// KONFIGURASI TABEL prestasi
	public function insert_prestasi($data){
        $this->db->insert("prestasi",$data);
    }
    
    public function update_prestasi($where,$data){
        $this->db->update("prestasi",$data,$where);
    }

    public function delete_prestasi($where){
        $this->db->delete("prestasi", $where);
    }

	public function get_prestasi($select, $where){
        $data = "";
		$this->db->select($select);
        $this->db->from("prestasi p");
        $this->db->join('tahun t', 'p.tahun_id = t.tahun_id', 'left');
		$this->db->where($where);
		$this->db->limit(1);
		$Q = $this->db->get();
		if ($Q->num_rows() > 0){
    ...
    public function grid_all_prestasi($select, $sidx, $sord, $limit="", $start, $where="", $like=""){
        $data = "";
        $this->db->select($select);
        $this->db->from("prestasi p");
        $this->db->join('tahun t', 'p.tahun_id = t.tahun_id', 'left');
    ...
    public function count_all_prestasi($where="", $like=""){
        $this->db->select("*");
